...
logging.basicConfig(format='%(levelname)s:%(message)s', filename="smallcorpus.log", encoding='utf-8', level=logging.WARNING, filemode='w')
...
def create_cooc_matrix(sentences,path_to_matrix,corpus_name):
...
if __name__ == "__main__":
    for corpus_2 in glob.glob(sys.argv[2]+"*.pkl"):
        ...
        #logger.warning(f"Loading cooccurrence matrices for {corpus_1_name} and {corpus_2_name}")
        with open(corpus_1, "rb") as f:
            create_cooc_matrix(pickle.load(f), path_to_matrix_1, corpus_1_name)
        with open(corpus_2, "rb") as f:
            create_cooc_matrix(pickle.load(f), path_to_matrix_2, corpus_2_name)
        ...
        sinr_1, sinr_vectors_1 = tf.compute_communities_and_vectors(path_to_matrix_1, corpus_1_name, ".pk", 80, logger)
        sinr_2, sinr_vectors_2 = tf.compute_communities_and_vectors(path_to_matrix_2, corpus_2_name, ".pk", 50, logger)

        # Evaluating the similarity for the small corpus using communities learned on itself
        #logger.warning(f"Evaluating similarity for {corpus_2_name}")
        logger.warning(tf.compute_similarity(sinr_vectors_2,corpus_2_name))
        ...
        sinr_transferred.transfert_communities_labels(sinr_vectors_1.get_communities_as_labels_sets(), graph_name=corpus_transferred)
        ...
        logger.warning(tf.compute_similarity(sinr_vectors_transferred, corpus_transferred))
        ...
        logger.warning(tf.compute_similarity(sinr_vectors_refined,corpus_refined))
